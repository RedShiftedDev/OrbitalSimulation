...
class CubeSphere : public Object3D {
public:
  explicit CubeSphere(float size = 1.0f, int resolution = 16);
  ~CubeSphere() override = default;
  ...
  const std::string &getType() const override {
    static const std::string type = "CubeSphere";
    return type;
  }

  // CubeSphere-specific methods
...
private:
  ...
  void buildIndices();
  void setupBuffers();
  glm::vec3 spherify(const glm::vec3 &point) const;
